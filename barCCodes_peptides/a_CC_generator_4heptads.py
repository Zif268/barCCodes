...
def peptide_seq(h):
    ...
    :param h: list of heptads (example: ["EN", "KI", "KI", "EI"])
    :return: dictionary with permutations (variants) as keys and full peptide sequences as values
    (example: {"AAA": SPEDENAALEAKIAALKAKIAALKAEIAALKAG, "AAQ": SPEDENAALEQKIAALKQKIAALKQEIAALKQG, ...}
    """
    sequences = dict()
    full_seqs = []
    variant_dict = dict()
    ...
    for m in list(zip(sequences.get(1), sequences.get(2), sequences.get(3), sequences.get(4))):
...
# dictionary of heptads combinations with assigned names
# {1: ["EI", "EI", "EI", "EI"], 2: ["EI", "EI", "EI", "EN"], ...}
pep_d = dict(zip(pep_names, pep))

# generate a full dictionary with all permutations (variants) for all generated peptides
# {("name, variant"): [["EN", "KN", "KI", "EI"], "SPEDENAALEAKNAALKAKIAALKAEIAALEAG"], ...}
full_dict = dict()
