...
dirname = os.path.join(origin, "Interaction_scores")
os.makedirs(dirname, exist_ok=True)
out_file_all = os.path.join(dirname, "CC_5heptads_all_scores-v2.1.csv.gz")
...
# mismatches
mm = [("EN", "EI"), ("EI", "EN"), ("KN", "KI"), ("KI", "KN"),
      ("EI", "KN"), ("KN", "EI"), ("KI", "EN"), ("EN", "KI")]

# types of interactions
int_type = {"pair": [5, 5],
            "shift_0:mismatch_1": [5, 4],
            "shift_0:mismatch_2": [5, 3],
            "shift_0:mismatch_3": [5, 2],
            "shift_0:mismatch_4": [5, 1],
            "shift_0:mismatch_5": [5, 0],
            "shift_1:mismatch_0": [4, 4],
            "shift_1:mismatch_1": [4, 3],
            "shift_1:mismatch_2": [4, 2],
            "shift_1:mismatch_3": [4, 1],
            "shift_1:mismatch_4": [4, 0],
            "shift_2:mismatch_0": [3, 3],
            "shift_2:mismatch_1": [3, 2],
            "shift_2:mismatch_2": [3, 1],
            "shift_2:mismatch_3": [3, 0],
            "shift_3:mismatch_0": [2, 2],
            "shift_3:mismatch_1": [2, 1],
            "shift_3:mismatch_2": [2, 0]}
...
def conf(p1, p2):
    ...
    :param p1: list of heptads in the form ["EI", "EI", "EI", "EI"] for peptide 1
    :param p2: list of heptads in the form ["EI", "EI", "EI", "EI"] for peptide 2
    :return: tuple of tuples with possible heptad pairings in the form
    ((("EI", "KI"), ("KI, "EN), ("EN", "KN")), (("EI, "EI"), ("KN", "KI")))
    """
    z0 = tuple(zip(p1, p2))
    ...
    z3l = tuple(zip(p1, p2[3:]))
    z3r = tuple(zip(p1[3:], p2))
    return z0, z1l, z1r, z2l, z2r, z3l, z3r
...
def dg_calc(t):
   ...
   """
    t_list = list(t)
    # fraying correction N-terminus:
    if t_list[0] in mm:
        t_list[0] = "mm_N"
    # fraying correction C-terminus:
    if t_list[-1] in mm:
        t_list[-1] = "mm_C"
    dg = math.fsum(coup_en.get(h) for h in t_list)
    return round(dg, 1)
...
def pairing(p1, p2):
    ...
    :param p1: list of heptads in the form ["EI", "EI", "EI", "EI"] for peptide 1
    :param p2: list of heptads in the form ["EI", "EI", "EI", "EI"] for peptide 2
    :return: dictionary with conformation tuples as keys and coupling energies (dg) as values
    """
...
def best_conf(cfs):
    ...
    :param cfs: dictionary with conformation tuples and coupling energies (output of pairing function)
...
for comb in bcf_comb:
    ...
    fname = os.path.join(indir, "CC_list_5heptads_" + str(ccset) + "_hel_filtered.csv")
    df = pd.read_csv(fname, header=0, index_col=0)

    peptides = dict()
    for i, row in df.iterrows():
        p = int(i.split(",")[0][1:])
        peptides[p] = [[row["Heptad1"], row["Heptad2"], row["Heptad3"], row["Heptad4"], row["Heptad5"]],
                       row["Sequence"], row["Helicity"]]

    # generate a csv file
    out_file = os.path.join(dirname, "CC_5heptads" + str(ccset) + "_scores-v2.1.csv.gz")
